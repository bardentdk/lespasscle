...
export default function DashboardApprenant() {
  const { id } = useParams(); // ID de l'apprenant (passé en URL)
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 2. Récupérer toutes les présences et les séances liées
      ...
      // Simulation d'une progression vers un objectif d'heures (ex: 150h pour le dispositif)
      const progressionCalculated = Math.min(100, (totalH / 150) * 100);
  ...
  const exportToPDF = async () => {
    ...
    try {
      const canvas = await html2canvas(dashboardRef.current, {
        ...
        useCORS: true,
        logging: false,
        backgroundColor: '#f9fafb' // Couleur de fond du PDF (gray-50)
      });
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="flex items-center px-5 py-2.5 bg-white border border-gray-200 text-gray-700 font-bold rounded-xl hover:bg-gray-50 transition-all shadow-sm hover:shadow-md disabled:opacity-50"
        >
        ...
        <div className="bg-gradient-to-r from-brand-600 to-purple-700 rounded-3xl p-8 text-white shadow-xl shadow-brand-100 flex flex-col md:flex-row items-center gap-8">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-4 mt-4">
              <span className="bg-white/10 px-3 py-1 rounded-full text-xs font-bold border border-white/20">Stagiaire Actif</span>
              <span className="bg-white/10 px-3 py-1 rounded-full text-xs font-bold border border-white/20">ID: {apprenant.id.slice(0,8)}</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold text-gray-500 uppercase">Heures validées</p>
                <p className="text-2xl font-black text-gray-900">{stats.totalHeures}h</p>
              </div>
            </div>
          </div>
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold text-gray-500 uppercase">Séances réalisées</p>
                <p className="text-2xl font-black text-gray-900">{stats.seancesEffectuees}</p>
              </div>
            </div>
          </div>
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold text-gray-500 uppercase">Taux d'assiduité</p>
                <p className="text-2xl font-black text-gray-900">{stats.tauxPresence}%</p>
              </div>
            </div>
          </div>
        </div>

        {/* Section Évolution et Liste */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4">
            <h3 className="text-lg font-bold text-gray-900 flex items-center">
              ...
              Historique des ateliers & séances
            </h3>
                  ...
                  {activites.map((item, idx) => (
                    <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4">
                        <p className="font-bold text-gray-900 text-sm">{item.seances.title}</p>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-500">
                        {format(parseISO(item.seances.date), 'dd MMM yyyy', { locale: fr })}
                      </td>
                      <td className="px-6 py-4 text-center">
                        <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${
                          item.status === 'present' ? 'bg-green-100 text-green-700' : 
                          item.status === 'absent' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'
                        }`}>
                          {item.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right font-bold text-gray-900 text-sm">
                        {item.heures_validees}h
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm flex flex-col items-center justify-center">
              ...
              <div className="relative h-40 w-40 flex items-center justify-center">
                <svg className="h-full w-full transform -rotate-90">
                  ...
                  <circle 
                    ...
                    strokeDasharray={440}
                    strokeDashoffset={440 - (440 * stats.progression) / 100}
                    strokeLinecap="round"
              ...
              <div className="mt-8 space-y-4 w-full">
                <div className="flex justify-between items-end">
                  <div>
                    <p className="text-xs font-bold text-gray-400 uppercase">Cumul actuel</p>
                    <p className="text-lg font-black text-gray-900">{stats.totalHeures}h</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-bold text-gray-400 uppercase">Cible</p>
                    <p className="text-lg font-black text-gray-600">150h</p>
                  </div>
                </div>
              ...
              <div className="mt-6 p-4 bg-amber-50 rounded-2xl border border-amber-100 flex items-start">
                ...
                <p className="text-xs text-amber-800 leading-relaxed font-medium">
                  Encore <strong>{150 - stats.totalHeures} heures</strong> pour atteindre l'objectif de fin de parcours.
                </p>
              </div>
            </div>
          </div>
